...
class Curator:
    ...
    Path(niraapad_path+"\\niraapad\\traces").mkdir(parents=True, exist_ok=True)
                ...
                if trace['Trace Message Type'] == "InitializeTraceMsg":
                    ...
                    if trace['Trace Message']['req']['backend_instance_id'] in self.backend_instance_id_magstr:
                        ...
                        if "data" in trace['Trace Message']['req']['args'].keys() and trace['Trace Message']['req']['args']['data']['command_name']: 
                            ...
                            elif "flags" in trace['Trace Message']['req']['args']['data'].keys():
                                writer.writerow([trace['Trace Message']['req']['id'],trace['_id'], "N9", trace['Trace Message']['req']['args']['data']['command_name'], str(trace['Trace Message']['req']['args']['data']['flags']).replace("{", "").replace("}", "").replace("'","").strip(','), trace['Trace Message']['resp']['resp'], trace['Trace Message']['resp']['exception'], trace['Trace Message']['profile']['exec_time_sec']])
                            else:
    ...
    def merging_csv(self, file):
        ...
        df3.to_csv(self.csv_path +  "\\" +file.strip(".json") + ".csv", index=False)
