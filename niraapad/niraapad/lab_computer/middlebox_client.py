...
class MiddleboxClient:
    ...
    def list_devices(self):
        ...
        for di in resp.devices_info:
            unpacked.append(SerialDeviceInfo(
                index=int(NoneIfEmptyString(di.index)),
                serial=di.serial,
    ...
    def list_devices_trace(self, serial_devices_info):
        ...
        self.stub.ListDevicesTrace(middlebox_pb2.ListDevicesTraceMsg(
            middlebox_pb2.ListDevicesResp(devices_info=packed)))

    def list_device_ports(self):
        resp = self.stub.ListDevicePorts(EmptyMsg())
        return resp.ports

    def list_device_ports_trace(self, ports):
        self.stub.ListDevicePortsTrace(middlebox_pb2.ListDevicePortsTraceMsg(
            middlebox_pb2.ListDevicePortsResp(ports=ports)))

    def list_device_serials(self):
        resp = self.stub.ListDeviceSerials(EmptyMsg())
    ...
    def list_device_serials_trace(self, serial_numbers):
        self.stub.ListDeviceSerialsTrace(middlebox_pb2.ListDeviceSerialsTraceMsg(
            middlebox_pb2.ListDeviceSerialsResp(serial_numbers=serial_numbers)))

    def initialize(self, resp, device, device_serial, device_number,
        ...
        request = middlebox_pb2.InitializeReq(
            ...
            connect=connect)
        resp = self.stub.Initialize(request)
    ...
    def init_device(self, device_id):
        self.stub.InitDevice(middlebox_pb2.InitDevice(device_id=device_id)
    ...
    def set_parameters(self, device_id, baudrate, parity, stop_bits, data_bits):
        self.stub.SetParameters(middlebox_pb2.SetParametersReq(
            device_id=device_id,
            baudrate=EmptyStringIfNone(baudrate),
            parity=EmptyStringIfNone(parity),
            stop_bits=EmptyStringIfNone(stop_bits),
            data_bits=EmptyStringIfNone(data_bits))
    ...
    def info(self, device_id):
        di = self.stub.Info(middlebox_pb2.InfoReq(device_id=device_id))
    ...
    def set_read_timeout(self, device_id, timeout):
        self.stub.ReadTimeout(middlebox_pb2.ReadTimeoutReq(
    ...
    def set_write_timeout(self, device_id, timeout):
        self.stub.WriteTimeout(middlebox_pb2.WriteTimeoutReq(
            device_id=device_id, timeout=timeout))
    ...
    def read_trace(self, device_id, num_bytes, timeout, data):
        self.stub.ReadTrace(middlebox_ob2.ReadTraceMsg(
    ...
    def read_line_trace(self, device_id, line_ending, timeout, data):
        self.stub.ReadLineTrace(middlebox_ob2.ReadLineTraceMsg(
    ...
    def write(self, device_id, data, timeout, num_bytes):
        data_format = middlebox_pb2.WriteReq.Format.STRING
        if type(data) == bytes:
            data = data.decode()
            data_format = middlebox_pb2.WriteReq.Format.BYTES

        self.stub.WriteTrace(middlebox_ob2.WriteTraceMsg(
    ...
    def request(self, device_id, write_data, timeout, line_ending):
        resp = self.stub.Req(middlebox_pb2.RequestReq(
            device_id=device_id,
            data=write_data,
            timeout=EmptyStringIfNone(timeout),
            line_ending=line_ending))
    ...
    def set_bit_mode(self, device_id, mask, enable):
        self.stub.ResetInputBuffer(middlebox_pb2.SetBitModeReq(
            device_id=device_id, mask=mask, enable=enable)

    def set_bit_mode_trace(self, device_id, mask, enable):
        self.stub.ResetInputBufferTrace(middlebox_pb2.SetBitModeTraceMsg(
