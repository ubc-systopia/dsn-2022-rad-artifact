...
def compute_transition_prob():
    ...
    li2 = []
    file_index1 = []
    t_regex = "Order3_MC_5CV_transition_"
    ...
    for train_index, test_index in kf.split(file_index2):
        ...
        df_transition = pd.read_csv(args.idir + t_regex + "fold" + str(fold) + ".csv", index_col=False, names = ["prev2_state","prev_state", "current_state", "next_state", "count"])

        # compute the probability of each transition
        df_transition['marginal'] = df_transition.groupby(["prev2_state",'prev_state','current_state'])['count'].transform('sum')
        ...
        for item in test:
            ...
            print(item)
            df_transition["prev2_state_clean"] = df_transition.apply(clean_prev2_state, axis=1)
            df_transition["prev_state_clean"] = df_transition.apply(clean_prev_state, axis=1)
            ...
            for i in range(1, len(test_procedure_list) - 1):
                prev2_cmd = test_procedure_list[i - 2]
                prev_cmd = test_procedure_list[i - 1]
                current_cmd = test_procedure_list[i]
                next_cmd = test_procedure_list[i + 1]

                # print(current_cmd, next_cmd, len(next_cmd))
                subset = df_transition.loc[(df_transition['prev2_state_clean'] == prev2_cmd.strip()) &
                                           (df_transition['prev_state_clean'] == prev_cmd.strip()) &
                                            (df_transition['current_state_clean'] == current_cmd.strip()) &
...
def clean_prev2_state(row):
    return(row["prev2_state"].strip())

def clean_prev_state(row):
    return(row["prev_state"].strip())
...
def main():
    ...
    plt.tight_layout()
    plt.savefig("./perplexity_score_3O_MC_5FCV.pdf")
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig("./perplexity_score_boxplot_3O_MC_5FCV.pdf")
    plt.show()
    ...
    #plt.ylim(0,6)
    plt.tight_layout()
    plt.savefig("./perplexity_score_boxplot_nohue_3O_MC_5FCV.pdf")
    plt.show()
...
Accuracy : 0.8
Weighted Accuracy : 0.8214285714285714 0.8305084745762712
True Negative Rate : 0.7727272727272727
**********************************************
"""
